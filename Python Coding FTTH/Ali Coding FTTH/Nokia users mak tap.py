...
def handle_session(hostname, username, password, port, olt_name, seq):
    ...
    for attempt in range(max_retries):
        try:
            ...
            df_output1 = df_output1.rename(columns={0: olt_name})  # Set the first column header to FDT

            # Create a DataFrame for Sheet2
           
            
            # Save the DataFrames for Sheet1 and Sheet2 to Excel
            df_output1.to_excel(writer, sheet_name=(olt_name), index=False)
...
for _, row in df.iterrows():
    ...
    excel_filename = rf'Nokia Script.xlsx'
